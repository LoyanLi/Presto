...
- 当前文档基线版本：`0.3.4`
- 发布说明：[docs/releases/v0.3.4-release.md](docs/releases/v0.3.4-release.md)

`0.3.4` 这一版当前最重要的事实有九条：
...
- 打包态 bundled Python runtime 现在自带所需 `Python.framework` 并清理构建机绝对路径，避免用户机器缺少本地 Python framework 时后端启动失败。
- 安装包现在把 bundled Python runtime 收口为“仅供 Presto 后端使用”的最小运行时，并继续裁剪冗余标准库、typing stub、bytecode 与开发产物，显著缩小 `.app` 与 `.dmg` 体积。
- Tauri 正式打包链现在会从 `src-tauri/resources/` 把 `backend`、`frontend`、`plugins` 资源显式同步回最终 `.app` bundle，避免生成只有壳层而没有运行时资源的错误安装包。
- Rust runtime 的本地 HTTP 健康检查已经修正；后端启动成功后不会再因为错误的 socket 半关闭而被宿主误判失败并清理掉。
- macOS 辅助功能权限现在会在每次启动时主动预检；任何依赖 Accessibility 的运行时调用在权限缺失时都会先弹出明确引导。
...
│   ├── desktop/             # 类型化 runtime client 装配
│   ├── runtime/             # 桌面宿主复用逻辑与历史实现参考
...
Rust command: runtime_invoke
        ...
        ├── plugins / automation / mobile progress
