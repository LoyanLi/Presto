...
- 当前文档基线版本：`0.3.1`
- 发布说明：[docs/releases/v0.3.1-release.md](docs/releases/v0.3.1-release.md)

`0.3.1` 这一版当前最重要的事实有三条：

- 宿主设置中心现在把 `Workflow Extensions` 和 `Automation Extensions` 收口到统一的 extension management 链路里。
- automation 首页卡片改为各自独立高度，不再互相拉伸。
- 官方 `batch-ara-backup-automation` 当前正式写入的是“批量复制备份 -> `.bak` 重命名 -> 隐藏并 inactive”这段备份链。
