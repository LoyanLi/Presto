...
test('tauri packaging script lays out a branded DMG Finder window', async () => {
  ...
  assert.match(packageBuildSource, /set dmgWindow to front window/)
  assert.match(packageBuildSource, /set target of dmgWindow to dmgFolder/)
  assert.match(packageBuildSource, /set current view of container window of dmgFolder to icon view/)
  assert.match(packageBuildSource, /set bounds of container window of dmgFolder to \{120, 120, \$\{120 \+ dmgWindowWidth\}, \$\{120 \+ dmgWindowHeight\}\}/)
  assert.match(packageBuildSource, /set iconViewOptions to icon view options of container window of dmgFolder/)
  assert.match(packageBuildSource, /set toolbar visible of dmgWindow to false/)
  assert.match(packageBuildSource, /set statusbar visible of dmgWindow to false/)
  assert.match(packageBuildSource, /set pathbar visible of dmgWindow to false/)
