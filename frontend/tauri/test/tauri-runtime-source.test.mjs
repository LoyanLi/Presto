...
test('tauri host uses invoke bridge without a packaged stdio sidecar RPC layer', async () => {
  ...
  assert.match(tauriRendererSource, /invoke\('app_ready'\)/)
  assert.match(tauriRendererSource, /onReady:/)
  ...
  assert.match(rustSource, /show\(\)/)
  assert.match(cargoToml, /\[package\]/)
  assert.match(cargoToml, /tauri/)
  assert.match(tauriConfig, /"visible": false/)
  assert.match(tauriConfig, /"label": "splashscreen"/)
  assert.match(tauriConfig, /"url": "\/splashscreen.html"/)
