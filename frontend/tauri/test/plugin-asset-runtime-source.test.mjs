...
test('tauri plugin loading uses asset URLs and enables asset protocol for installed extensions', async () => {
  ...
  assert.match(pluginHostRuntimeSource, /convertFileSrc\(pathValue\)/)
  assert.match(pluginHostRuntimeSource, /new URL\(encodeTauriAssetPath\(pathValue\), runtimeAssetUrl\)/)
