...
async function createLocalizedWorkflowPluginFixture(root) {
  const pluginRoot = path.join(root, 'official.localized-export-workflow')
  const entryPath = path.join(pluginRoot, 'dist/index.mjs')
  await mkdir(path.dirname(entryPath), { recursive: true })

  const manifest = {
    pluginId: 'official.localized-export-workflow',
    extensionType: 'workflow',
    version: '1.0.0',
    hostApiVersion: '0.1.0',
    supportedDaws: ['pro_tools'],
    uiRuntime: 'react18',
    displayName: 'Localized Export Workflow',
    description: 'Localized export workflow plugin.',
    entry: 'dist/index.mjs',
    pages: [
      {
        pageId: 'localized-export.page.main',
        path: '/plugins/localized-export',
        title: 'Localized Export Workflow',
        mount: 'workspace',
        componentExport: 'LocalizedExportWorkflowPage',
      },
    ],
    settingsPages: [
      {
        pageId: 'localized-export.page.settings',
        title: 'Localized Export Workflow',
        order: 10,
        storageKey: 'settings.v1',
        loadExport: 'loadLocalizedExportSettings',
        saveExport: 'saveLocalizedExportSettings',
        defaults: {
  ...
  await writeFile(
    ...
    `
      ...
      export function LocalizedExportWorkflowPage() {
        return null
      }

      export async function loadLocalizedExportSettings() {
        return { defaultSnapshotSelection: 'all' }
      }

      export async function saveLocalizedExportSettings(_storage, settings) {
        return settings
      }
    `,
  )
...
test('loadHostPlugins resolves plugin-owned localized manifests before building host entries', async (t) => {
  const { loadHostPlugins } = await loadPluginHostRuntime()
  const sandbox = await mkdtemp(path.join(tmpdir(), 'presto-plugin-host-runtime-localized-'))
  const plugin = await createLocalizedWorkflowPluginFixture(sandbox)
  ...
  assert.equal(result.managerModel.plugins[0]?.displayName, '导出流程')
  assert.equal(result.managerModel.plugins[0]?.description, '官方导出流程插件。')
  assert.equal(result.homeEntries[0]?.title, '导出流程')
  assert.equal(result.homeEntries[0]?.description, '官方导出流程插件。')
  assert.equal(result.pages[0]?.title, '导出流程')
  assert.equal(result.managerModel.settingsEntries[0]?.title, '导出流程')
  assert.equal(result.managerModel.settingsEntries[0]?.sections[0]?.title, '默认快照选择')
  assert.equal(result.managerModel.settingsEntries[0]?.sections[0]?.fields[0]?.label, '默认选中全部快照')
})
