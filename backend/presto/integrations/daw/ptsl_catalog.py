...
_COMMANDS = tuple(
    PtslCommandCatalogEntry(
        ...
        request_message=str(entry["request_message"]) if entry.get("request_message") else None,
        response_message=str(entry["response_message"]) if entry.get("response_message") else None,
