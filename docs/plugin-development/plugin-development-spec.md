...
本文档只描述当前 `0.3.11` 代码已经成立的插件协议，并保留所有插件共享的规则。`workflow`、`automation`、`tool`、UI、自动化内核的专项要求已经拆到独立页面。
...
1. 插件执行正式业务动作时，必须走 `context.presto.*` 或工具 runner 的受限执行接口
2. 插件权限由 `requiredCapabilities` 与 `toolRuntimePermissions` 决定
...
- `tool` 插件通常会补 `tools[]`、tools 页模块和 `bundledResources`
...
tool runner 的上下文会在 `PluginContext` 基础上增加：
...
- tool runtime 声明：`toolRuntimePermissions`
...
2. manifest 解析
3. manifest 字段校验
4. workflow definition 校验（仅 `workflow`）
5. 入口模块可加载校验

`tool` 插件在 manifest 校验阶段还会检查：
...
- 如果你在写 workflow 页面和 settings：读 [Workflow 插件规范](workflow-plugin-standard.md) 和 [插件 UI 规范](plugin-ui-standard.md)
- 如果你在写自动化卡片类插件：读 [Automation 插件规范](automation-plugin-standard.md)
- 如果你在写工具型插件：读 [Tool 插件规范](tool-plugin-standard.md) 和 [插件 UI 规范](plugin-ui-standard.md)
