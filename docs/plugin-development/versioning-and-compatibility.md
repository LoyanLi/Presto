...
- App Version：当前仓库值是 `0.3.12`
- App Name：`Presto`
- Backend FastAPI Version：由 `backend/presto/version.py::VERSION` 导出，当前值也是 `0.3.12`
...
这只是当前实现里的兼容白名单，不代表项目已经完成长期多代兼容治理。官方插件当前仍统一使用 `0.1.0`。
